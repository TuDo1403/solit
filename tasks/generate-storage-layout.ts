...
const preprocessFile = (fileContent: string) => {
    ...
    const fileContentWithoutWhiteSpace = fileContent.replace(
        whiteSpaceRegex,
        "",
    );

    // get only table contents
    const startIndex = fileContentWithoutWhiteSpace.indexOf(
        TABLE_STYLE.headerTop.right,
    );
    const endIndex = fileContentWithoutWhiteSpace.indexOf(
        TABLE_STYLE.tableBottom.left,
    );
    if (startIndex !== -1 && endIndex !== -1 && startIndex < endIndex) {
        const result = fileContentWithoutWhiteSpace.substring(
            startIndex + 2,
            endIndex,
        );
        return result;
...
const preprocessTable = (tableContent: string) => {
    ...
    const listItemOfTable = contentsWithoutColorCode.split(
...
const removeIdentifierSuffix = (type: string) => {
    ...
    return type
        .replace(suffixIdRegex, "_$1")
        .replace(contractRegex, "$1($2)")
        .replace(enumRegex, "$1($2)");
};

const generateStorageLayoutTable = ({
    source,
    destination,
}: {
    source: string;
    ...
    try {
        if (fs.existsSync(source)) {
            ...
            for (let i = 0; i < listItemOfTable.length; i += 9) {
                ...
                const row = listItemOfTable
                    .slice(i, i + 8)
                    .filter((_, idx) => idx != 5 && idx != 6);
            ...
            console.log(
                `Successful generate storage layout table at ${destination}`,
            );
        } else {
...
const generateStorageLayoutInline = ({
    source,
    destination,
    override,
}: {
    ...
    try {
        if (fs.existsSync(source)) {
            if (!fs.existsSync(destination) || override) {
                const logger = fs.createWriteStream(destination, {
                    flags: "w",
                });
                const fileContent = fs.readFileSync(source, "utf-8");
                const tableContent = preprocessFile(fileContent);
...
const removeTempStorageLayout = ({ path }: { path: string }) => {
    try {
...
task("generate-storage-layout-table")
    .addParam(
        "source",
        "The path to storage layout file extracted from hardhat-storage-layout",
    )
    .addOptionalParam(
        "destination",
        "The path to store storage layout after generating",
        "logs/storage_layout_table.log",
    )
    .setAction(async ({ source, destination }, _) => {
        generateStorageLayoutTable({ source, destination });
        removeTempStorageLayout({ path: source });
    });
...
task("generate-storage-layout")
    ...
    .addOptionalParam(
        "override",
        "Indicates whether override the destination if it already exits",
        true,
        boolean,
    )
    .setAction(async ({ source, override }, _) => {
        try {
            generateStorageLayoutTable({
                source,
                destination: "logs/storage_layout_table.log",
            });
            generateStorageLayoutInline({
                source,
                override,
                destination: "logs/storage_layout.log",
            });
            removeTempStorageLayout({ path: source });
        } catch (err) {
            console.error(err);
