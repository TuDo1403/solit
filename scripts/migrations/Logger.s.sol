...
contract Logger {
    ...
    function generateDeploymentArtifact(
        ...
        string memory dirPath = _config.getDeploymentDirectory(
            _config.getCurrentNetwork()
        );
        ...
        if (_vm.exists(filePath)) {
            ...
            if (_vm.keyExists(existedJson, ".numDeployments")) {
                numDeployments = _vm.parseJsonUint(
        ...
        json.serialize(
            "bytecode",
            item.at('"bytecode"').at('"object"').value()
        );
        json.serialize(
            "deployedBytecode",
            item.at('"deployedBytecode"').at('"object"').value()
        );
        ...
        console2.log(
            string.concat(fileName, " deployed at:").green(),
            contractAddr.green()
        );
    }
}
