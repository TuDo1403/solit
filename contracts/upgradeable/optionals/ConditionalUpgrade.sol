    ...
    IConditionalUpgrade
{
    ...
    modifier whenSatisfied() {
        _;
        if (_isSatisfy()) {
            try this.selfUpgrade{ gas: _gasStipned() }() {}
    ...
    function _getCurrentImplementation() internal view virtual returns (address) {
    ...
    function _dispatchCall(address impl_) internal virtual whenSatisfied returns (bytes memory result) 
    {
