...
library MerkleProof {
    ...
    function verifyMultiProof(
        ...
        assembly {
            ...
            for {
            ...
            } eq(add(leafsLength, proofLength), add(flagsLength, 1)) {
                ...
                if iszero(flagsLength) {
                    ...
                    isValid := eq(
                        mload(xor(leafs, mul(xor(proof, leafs), proofLength))),
                        root
                    )
                    break
                }
                ...
                for {
                    ...
                    if iszero(lt(hashesBack, flagsLength)) {
                        break
                    }
                }
    ...
    function verifyMultiProofCalldata(
        ...
        assembly {
            ...
            for {
            ...
            } eq(add(leafs.length, proof.length), add(flags.length, 1)) {
                ...
                for {
                    ...
                    mstore(hashesBack, keccak256(0x00, 0x40))
                    hashesBack := add(hashesBack, 0x20)
                    if iszero(lt(hashesBack, flags.length)) {
                        break
                    }
                }
                // Checks if the last value in the queue is same as the root.
